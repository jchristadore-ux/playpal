// statsService.js — per-round and lifetime performance statistics.
//
...
const StatsService = (function () {
  ...
  function roundDataFromSnapshot(snap) {
    if (!snap) return null;
    const round = snap.round || snap;
    if (!round || !round.course || !round.players) return null;

    let scores = snap.scores || null;
    let putts  = snap.putts  || round.putts || {};
    if (!scores && round.holeScores) {
      scores = {};
      putts = putts || {};
  ...
  function _scoreCounts(diffsWithPar) {
    const c = { aces: 0, eagles: 0, birdies: 0, pars: 0, bogeys: 0, doubles: 0, triplePlus: 0 };
    diffsWithPar.forEach(({ gross, par }) => {
      if (gross === 1) c.aces++;
      const d = gross - par;
      if (d <= -2) c.eagles++;
      else if (d === -1) c.birdies++;
      else if (d === 0) c.pars++;
      else if (d === 1) c.bogeys++;
      else if (d === 2) c.doubles++;
      else c.triplePlus++;
    });
    return c;
  }

  // Full stat line for one player in one round.
  function computePlayerRound(data, pid) {
    const holes = data.course.holes || [];
    const scores = data.scores[pid] || [];
    const putts  = data.putts[pid]  || [];
    const fir    = data.firData[pid] || [];
    const gir    = data.girData[pid] || [];
    ...
    holes.forEach((h, i) => {
      const g = scores[i];
      if (g) {
        gross += g; par += h.par; played++;
        if (i < 9) front += g; else back += g;
      ...
      if (h.par !== 3 && fir[i] !== null && fir[i] !== undefined) { firEligible++; if (fir[i]) firHit++; }
    ...
    // Best nine: lower of front/back vs their par (18-hole, fully played halves only)
    if (holes.length === 18) {
      const frontDone = scores.slice(0, 9).every(s => s > 0);
      const backDone  = scores.slice(9).every(s => s > 0);
      const frontPar = holes.slice(0, 9).reduce((a, h) => a + h.par, 0);
      const backPar  = holes.slice(9).reduce((a, h) => a + h.par, 0);
      const candidates = [];
      if (frontDone) candidates.push({ label: 'Front 9', gross: front, toPar: front - frontPar });
      if (backDone)  candidates.push({ label: 'Back 9',  gross: back,  toPar: back - backPar });
    ...
    return {
      ...
      parAverages: {
        3: parBuckets[3].n ? parBuckets[3].total / parBuckets[3].n : null,
        4: parBuckets[4].n ? parBuckets[4].total / parBuckets[4].n : null,
        5: parBuckets[5].n ? parBuckets[5].total / parBuckets[5].n : null,
      },
      bestNine,
    };
  }

  // Lifetime aggregate for one player across many round-data records
  // (newest-first or any order; trend comes back oldest-first).
  function aggregatePlayer(dataList, pid) {
    const rounds = [];
    dataList.forEach(data => {
      if (!data || !data.scores || !data.scores[pid]) return;
      const r = computePlayerRound(data, pid);
    ...
    rounds.sort((a, b) => (a.savedAt || 0) - (b.savedAt || 0));

    const complete = rounds.filter(r => r.stats.complete && r.stats.holeCount === 18);
    const sum = (fn) => rounds.reduce((a, r) => a + fn(r.stats), 0);
    const totals = {
      aces:    sum(s => s.counts.aces),
      eagles:  sum(s => s.counts.eagles),
      birdies: sum(s => s.counts.birdies),
      pars:    sum(s => s.counts.pars),
    ...
    const best = (list, fn, lower) => {
      ...
      list.forEach(r => {
        ...
        if (!out || (lower ? v < out.value : v > out.value)) {
          out = { value: v, courseName: r.courseName, date: r.date, syncCode: r.syncCode };
        }
      });
      return out;
    };
    ...
    return {
      ...
      avgGross: complete.length ? complete.reduce((a, r) => a + r.stats.gross, 0) / complete.length : null,
      avgToPar: rounds.length ? rounds.reduce((a, r) => a + r.stats.toPar, 0) / rounds.length : null,
      totals,
      ...
      bests: {
        gross:  best(complete, s => s.gross, true),
        ...
        bestNine: (() => {
          let out = null;
          rounds.forEach(r => {
            const bn = r.stats.bestNine;
            if (!bn) return;
            if (!out || bn.toPar < out.value) out = { value: bn.toPar, gross: bn.gross, label: bn.label, courseName: r.courseName, date: r.date };
          });
          return out;
        })(),
      },
      parAverages: (() => {
        const acc = { 3: { n: 0, t: 0 }, 4: { n: 0, t: 0 }, 5: { n: 0, t: 0 } };
        rounds.forEach(r => [3, 4, 5].forEach(p => {
          const v = r.stats.parAverages[p];
          if (v !== null) { acc[p].n++; acc[p].t += v; }
        }));
        return {
          3: acc[3].n ? acc[3].t / acc[3].n : null,
          4: acc[4].n ? acc[4].t / acc[4].n : null,
          5: acc[5].n ? acc[5].t / acc[5].n : null,
        };
      })(),
      trend: rounds.map(r => ({
        gross: r.stats.gross,
        toPar: r.stats.toPar,
        complete: r.stats.complete,
  ...
  // Side-by-side hole comparison of one player across two rounds.
  function compareRounds(dataA, dataB, pid) {
    const holesA = dataA.course.holes || [];
    const holesB = dataB.course.holes || [];
    const n = Math.max(holesA.length, holesB.length);
    const rows = [];
    for (let i = 0; i < n; i++) {
      const a = (dataA.scores[pid] || [])[i] || null;
      const b = (dataB.scores[pid] || [])[i] || null;
      rows.push({
        num: i + 1,
        parA: holesA[i] ? holesA[i].par : null,
        parB: holesB[i] ? holesB[i].par : null,
        a, b,
        diff: a && b ? a - b : null,
      });
    }
    const statsA = computePlayerRound(dataA, pid);
    const statsB = computePlayerRound(dataB, pid);
    return { rows, a: statsA, b: statsB };
  }

  return {
    roundDataFromSnapshot,
    computePlayerRound,
    aggregatePlayer,
    compareRounds,
  };
