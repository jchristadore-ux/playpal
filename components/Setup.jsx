// Setup.jsx v4 — Fixed course persistence + scorecard image scan feature
      ...
      <div
        ...
        {preview && (
          <img src={preview} alt="preview" style={{
            ...
            objectFit:'cover', opacity: status === 'scanning' ? 0.25 : 0.45,
          }}/>
        )}

        <div style={{position:'relative', zIndex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:8, padding:'0 20px', textAlign:'center'}}>
          {status === 'idle' && (
            <>
              <div style={{fontSize:40}}>📸</div>
              <div style={{fontFamily:'Barlow Condensed', fontWeight:700, fontSize:16, color:'#fff', letterSpacing:0.5}}>
                TAP TO UPLOAD SCORECARD
              </div>
              <div style={{fontFamily:'DM Sans', fontSize:12, color:'#4A6890'}}>
                or drag & drop · JPG, PNG, HEIC, screenshot
              </div>
            </>
          )}
          {status === 'scanning' && (
            <>
              <div style={{width:38, height:38, border:'3px solid #1E3A6E', borderTopColor:'#C9A84C', borderRadius:'50%', animation:'ppSpin 0.8s linear infinite'}}/>
              <div style={{fontFamily:'Barlow Condensed', fontWeight:700, fontSize:16, color:'#C9A84C', letterSpacing:1}}>
                READING SCORECARD…
              </div>
              <div style={{fontFamily:'DM Sans', fontSize:12, color:'#7A98BC'}}>Claude is extracting hole data</div>
            </>
          )}
          {status === 'done' && (
            <>
              <div style={{fontSize:36}}>✅</div>
              <div style={{fontFamily:'Barlow Condensed', fontWeight:700, fontSize:16, color:'#3DCB6C', letterSpacing:0.5}}>
                SCORECARD READ — REVIEW BELOW
              </div>
            </>
          )}
          {status === 'error' && (
            <>
              ...
              <div style={{fontFamily:'Barlow Condensed', fontWeight:700, fontSize:14, color:'#E5534B', letterSpacing:0.5}}>
                {errorMsg}
              </div>
              <div style={{fontFamily:'DM Sans', fontSize:12, color:'#7A98BC'}}>Tap to try a different image</div>
            </>
          )}
        </div>
      </div>
...
const CourseBuilder = ({ onSave, onCancel, prefill }) => {
  ...
  // ── PERSISTENCE FIX ───────────────────────────────────────────────────────
  // Build the full updated array here, write it atomically to both localStorage
  // and RTDB. Pass the complete array back to the parent so React state is
  // immediately correct without a round-trip read.
  const handleSave = () => {
    const course = {
      id:       'custom_' + Date.now(),
      name:     name.trim(),
...
const StakesInput = ({ value, onChange }) => {
  ...
  const selectPreset = (v) => { setCustom(false); setCustomVal(''); onChange(v); };
...
const SetupScreen = ({ allPlayers, onStart }) => {
  ...
  // ── CUSTOM COURSES — single source of truth ───────────────────────────────
  ...
  // On mount: pull RTDB, merge with any local-only courses
  React.useEffect(() => {
    if (!window.CourseSyncService) return;
    window.CourseSyncService.load(function(remote) {
  ...
  // CourseBuilder calls back with (newCourse, fullUpdatedArray)
  // Both localStorage and RTDB have already been written by CourseBuilder.
  // Just update React state here — no second write needed.
