...
const CourseBuilder = ({ onSave, onCancel }) => {
  ...
  const handleSave = () => {
    ...
    // Sync to RTDB so other devices see the custom course
    if (window.CourseSyncService) {
      window.CourseSyncService.save(saved, function() {});
    }
    onSave(course);
  };
...
const SetupScreen = ({ allPlayers, onStart }) => {
  ...
  // Load custom courses from RTDB on mount (cross-device sync)
  React.useEffect(() => {
    if (window.CourseSyncService) {
      window.CourseSyncService.load(function(remote) {
        if (remote && remote.length > 0) {
          setCustomCourses(remote);
          localStorage.setItem('pp_custom_courses', JSON.stringify(remote));
        }
      });
    }
  }, []);
  ...
  const njCourses     = filtered.filter(c => c.custom || c.location.includes(', NJ'));
  const otherCourses  = filtered.filter(c => !c.custom && !c.location.includes(', NJ'));
