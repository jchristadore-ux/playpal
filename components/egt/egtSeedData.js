// egtSeedData.js — the canonical EGT 2026 trip definition, embedded so the
// offline PWA imports it with no network fetch. Generated from
// fixtures/egt-2026-seed.json (the single source of truth). Re-generate with:
//   node scripts/gen-seed.mjs   — Do not hand-edit.
const EGT_SEED = {
  "schemaVersion": "2.3",
  "generatedFor": "PlayPal import \u2014 self-contained, all 6 courses loaded (White tees)",
  "loadContract": {
    "description": "Importer reads courseLibrary + rounds + strokeAllocations to auto-populate each round.",
    "perRoundAutoLoad": [
      "format",
      "each player's course handicap",
      "each player's pops + exact holes"
    ],
    "allTeesPlayed": "White",
    "allCoursesLoaded": true,
    "strokeIndexStatus": {
      "minerals": true,
      "ballyowen": true,
      "wildturkey": true,
      "crystalsprings": true,
      "cascades": true,
      "blackbear": true
    }
  },
  "trip": {
    "id": "egt-2026",
    "name": "EGT 2026 CUP",
    "dates": {
      "start": "2026-07-21",
      "end": "2026-07-24"
    },
    "venue": "Crystal Springs Resort \u2014 Hamburg / Vernon / Hardyston / Franklin, NJ",
    "lodging": "Airbnb \u2014 5 Magnum Trail, Warwick, NY",
    "holes": 108
  },
  "players": [
    {
      "id": "john",
      "name": "John",
      "handicapIndex": 18
    },
    {
      "id": "brian",
      "name": "Brian",
      "handicapIndex": 23
    },
    {
      "id": "tj",
      "name": "TJ",
      "handicapIndex": 28
    },
    {
      "id": "mike",
      "name": "Mike",
      "handicapIndex": 28
    }
  ],
  "handicapAllowances": {
    "singlesMatch": 1,
    "fourBallMatch": 0.9,
    "fourBallStableford": 0.85,
    "scramble2": {
      "low": 0.35,
      "high": 0.15
    },
    "foursomes": 0.5,
    "wolf": 1,
    "nines": 1,
    "skinsNet": 1,
    "courseHandicapFormula": "round(HI*Slope/113 + (CR-Par)); White ratings for the two-loop 9s are the 18-hole figures",
    "strokeAllocationRule": "distribute onto holes by 18-hole SI (2nd stroke where strokes>18); 9-hole loop games use 9-hole SI over that loop"
  },
  "courseLibrary": {
    "minerals": {
      "courseId": "minerals",
      "name": "Minerals Golf Club",
      "location": "Vernon, NJ",
      "designer": "Robert Trent Jones",
      "holesCount": 9,
      "loopsForRound": 2,
      "tees": [
        {
          "name": "Blue",
          "cr": 62.8,
          "slope": 104,
          "par": 62,
          "yards": 4610,
          "nine": {
            "cr": 31.4,
            "slope": 104,
            "par": 31
          }
        },
        {
          "name": "White",
          "cr": 60.2,
          "slope": 99,
          "par": 62,
          "yards": 4088,
          "nine": {
            "cr": 30.1,
            "slope": 99,
            "par": 31,
            "crDerived": true
          }
        }
      ],
      "playedTee": "White",
      "par": 62,
      "holes": [
        {
          "hole": 1,
          "par": 4,
          "si": 9,
          "whiteYards": 316
        },
        {
          "hole": 2,
          "par": 3,
          "si": 11,
          "whiteYards": 136
        },
        {
          "hole": 3,
          "par": 4,
          "si": 7,
          "whiteYards": 337
        },
        {
          "hole": 4,
          "par": 3,
          "si": 3,
          "whiteYards": 150
        },
        {
          "hole": 5,
          "par": 4,
          "si": 13,
          "whiteYards": 292
        },
        {
          "hole": 6,
          "par": 3,
          "si": 5,
          "whiteYards": 186
        },
        {
          "hole": 7,
          "par": 4,
          "si": 1,
          "whiteYards": 374
        },
        {
          "hole": 8,
          "par": 3,
          "si": 17,
          "whiteYards": 153
        },
        {
          "hole": 9,
          "par": 3,
          "si": 15,
          "whiteYards": 100
        },
        {
          "hole": 10,
          "par": 4,
          "si": 10,
          "whiteYards": 316
        },
        {
          "hole": 11,
          "par": 3,
          "si": 12,
          "whiteYards": 136
        },
        {
          "hole": 12,
          "par": 4,
          "si": 8,
          "whiteYards": 337
        },
        {
          "hole": 13,
          "par": 3,
          "si": 4,
          "whiteYards": 150
        },
        {
          "hole": 14,
          "par": 4,
          "si": 14,
          "whiteYards": 292
        },
        {
          "hole": 15,
          "par": 3,
          "si": 6,
          "whiteYards": 186
        },
        {
          "hole": 16,
          "par": 4,
          "si": 2,
          "whiteYards": 374
        },
        {
          "hole": 17,
          "par": 3,
          "si": 18,
          "whiteYards": 153
        },
        {
          "hole": 18,
          "par": 3,
          "si": 16,
          "whiteYards": 100
        }
      ],
      "strokeIndexVerified": true,
      "note": "Par-31 nine played twice (par 62). Card prints an 18-hole SI (loop 1 odd, loop 2 even), used verbatim. 9-hole White CR 30.1 derived as half the 18-hole 60.2 for the loop-2 Nines game.",
      "source": "Minerals printed scorecard (18-hole White 60.2/99)"
    },
    "ballyowen": {
      "courseId": "ballyowen",
      "name": "Ballyowen",
      "location": "Hamburg, NJ",
      "designer": "Roger Rulewich",
      "holesCount": 18,
      "tees": [
        {
          "name": "Black",
          "cr": 74.1,
          "slope": 134,
          "par": 72,
          "yards": 7094
        },
        {
          "name": "Gold",
          "cr": 71.5,
          "slope": 130,
          "par": 72,
          "yards": 6508
        },
        {
          "name": "Blue",
          "cr": 69.4,
          "slope": 125,
          "par": 72,
          "yards": 6093
        },
        {
          "name": "White",
          "cr": 66.9,
          "slope": 114,
          "par": 72,
          "yards": 5630
        }
      ],
      "playedTee": "White",
      "par": 72,
      "holes": [
        {
          "hole": 1,
          "par": 4,
          "si": 13
        },
        {
          "hole": 2,
          "par": 4,
          "si": 11
        },
        {
          "hole": 3,
          "par": 5,
          "si": 3
        },
        {
          "hole": 4,
          "par": 3,
          "si": 17
        },
        {
          "hole": 5,
          "par": 5,
          "si": 5
        },
        {
          "hole": 6,
          "par": 3,
          "si": 15
        },
        {
          "hole": 7,
          "par": 4,
          "si": 1
        },
        {
          "hole": 8,
          "par": 4,
          "si": 7
        },
        {
          "hole": 9,
          "par": 4,
          "si": 9
        },
        {
          "hole": 10,
          "par": 5,
          "si": 12
        },
        {
          "hole": 11,
          "par": 3,
          "si": 18
        },
        {
          "hole": 12,
          "par": 4,
          "si": 14
        },
        {
          "hole": 13,
          "par": 4,
          "si": 8
        },
        {
          "hole": 14,
          "par": 4,
          "si": 4
        },
        {
          "hole": 15,
          "par": 3,
          "si": 16
        },
        {
          "hole": 16,
          "par": 4,
          "si": 2
        },
        {
          "hole": 17,
          "par": 5,
          "si": 10
        },
        {
          "hole": 18,
          "par": 4,
          "si": 6
        }
      ],
      "strokeIndexVerified": true,
      "source": "18Birdies official scorecard"
    },
    "wildturkey": {
      "courseId": "wildturkey",
      "name": "Wild Turkey",
      "location": "Hamburg, NJ",
      "designer": "Roger Rulewich",
      "holesCount": 18,
      "tees": [
        {
          "name": "Black",
          "cr": 75.3,
          "slope": 135,
          "par": 71,
          "yards": 7202
        },
        {
          "name": "Blue",
          "cr": 71.7,
          "slope": 132,
          "par": 71,
          "yards": 6555
        },
        {
          "name": "White",
          "cr": 69,
          "slope": 129,
          "par": 71,
          "yards": 5942
        },
        {
          "name": "Red",
          "cr": 68.3,
          "slope": 114,
          "par": 71,
          "yards": 5024
        }
      ],
      "playedTee": "White",
      "par": 71,
      "holes": [
        {
          "hole": 1,
          "par": 4,
          "si": 11
        },
        {
          "hole": 2,
          "par": 3,
          "si": 17
        },
        {
          "hole": 3,
          "par": 5,
          "si": 5
        },
        {
          "hole": 4,
          "par": 4,
          "si": 1
        },
        {
          "hole": 5,
          "par": 4,
          "si": 3
        },
        {
          "hole": 6,
          "par": 4,
          "si": 13
        },
        {
          "hole": 7,
          "par": 3,
          "si": 7
        },
        {
          "hole": 8,
          "par": 5,
          "si": 15
        },
        {
          "hole": 9,
          "par": 4,
          "si": 9
        },
        {
          "hole": 10,
          "par": 3,
          "si": 10
        },
        {
          "hole": 11,
          "par": 5,
          "si": 2
        },
        {
          "hole": 12,
          "par": 4,
          "si": 14
        },
        {
          "hole": 13,
          "par": 4,
          "si": 16
        },
        {
          "hole": 14,
          "par": 3,
          "si": 12
        },
        {
          "hole": 15,
          "par": 4,
          "si": 4
        },
        {
          "hole": 16,
          "par": 3,
          "si": 18
        },
        {
          "hole": 17,
          "par": 5,
          "si": 8
        },
        {
          "hole": 18,
          "par": 4,
          "si": 6
        }
      ],
      "strokeIndexVerified": true,
      "source": "18Birdies official scorecard"
    },
    "crystalsprings": {
      "courseId": "crystalsprings",
      "name": "Crystal Springs Golf Club",
      "location": "Hardyston, NJ",
      "designer": "Robert von Hagge",
      "holesCount": 18,
      "tees": [
        {
          "name": "Black",
          "cr": 73.5,
          "slope": 139,
          "par": 72,
          "yards": 6816
        },
        {
          "name": "Blue",
          "cr": 71.4,
          "slope": 132,
          "par": 72,
          "yards": 6395
        },
        {
          "name": "White",
          "cr": 69.1,
          "slope": 123,
          "par": 72,
          "yards": 5862
        }
      ],
      "playedTee": "White",
      "par": 72,
      "holes": [
        {
          "hole": 1,
          "par": 5,
          "si": 7,
          "whiteYards": 503
        },
        {
          "hole": 2,
          "par": 4,
          "si": 3,
          "whiteYards": 356
        },
        {
          "hole": 3,
          "par": 4,
          "si": 1,
          "whiteYards": 383
        },
        {
          "hole": 4,
          "par": 5,
          "si": 11,
          "whiteYards": 403
        },
        {
          "hole": 5,
          "par": 3,
          "si": 17,
          "whiteYards": 113
        },
        {
          "hole": 6,
          "par": 4,
          "si": 9,
          "whiteYards": 343
        },
        {
          "hole": 7,
          "par": 4,
          "si": 13,
          "whiteYards": 366
        },
        {
          "hole": 8,
          "par": 5,
          "si": 5,
          "whiteYards": 493
        },
        {
          "hole": 9,
          "par": 3,
          "si": 15,
          "whiteYards": 130
        },
        {
          "hole": 10,
          "par": 4,
          "si": 10,
          "whiteYards": 347
        },
        {
          "hole": 11,
          "par": 3,
          "si": 14,
          "whiteYards": 160
        },
        {
          "hole": 12,
          "par": 5,
          "si": 6,
          "whiteYards": 475
        },
        {
          "hole": 13,
          "par": 3,
          "si": 16,
          "whiteYards": 125
        },
        {
          "hole": 14,
          "par": 5,
          "si": 2,
          "whiteYards": 524
        },
        {
          "hole": 15,
          "par": 3,
          "si": 18,
          "whiteYards": 105
        },
        {
          "hole": 16,
          "par": 4,
          "si": 4,
          "whiteYards": 356
        },
        {
          "hole": 17,
          "par": 4,
          "si": 8,
          "whiteYards": 350
        },
        {
          "hole": 18,
          "par": 4,
          "si": 12,
          "whiteYards": 330
        }
      ],
      "strokeIndexVerified": true,
      "note": "Card indexes each nine 1-9; 18-hole SI derived by standard front-odd/back-even interleave (front is the longer/higher-par nine). Flip if the official 18-hole SI differs.",
      "source": "Crystal Springs printed scorecard; 18-hole SI derived from per-nine indexes"
    },
    "cascades": {
      "courseId": "cascades",
      "name": "Cascades Golf Club",
      "location": "Hamburg, NJ",
      "designer": "Roger Rulewich",
      "holesCount": 9,
      "loopsForRound": 2,
      "tees": [
        {
          "name": "Black",
          "cr": 75.4,
          "slope": 141,
          "par": 72,
          "yards": 7254,
          "nine": {
            "cr": 37.7,
            "slope": 141,
            "par": 36
          }
        },
        {
          "name": "Blue",
          "cr": 72.6,
          "slope": 132,
          "par": 72,
          "yards": 6630,
          "nine": {
            "cr": 36.3,
            "slope": 132,
            "par": 36
          }
        },
        {
          "name": "White",
          "cr": 69,
          "slope": 124,
          "par": 72,
          "yards": 6044,
          "nine": {
            "cr": 34.5,
            "slope": 124,
            "par": 36
          }
        }
      ],
      "playedTee": "White",
      "par": 72,
      "holes": [
        {
          "hole": 1,
          "par": 4,
          "si": 7,
          "whiteYards": 368
        },
        {
          "hole": 2,
          "par": 4,
          "si": 5,
          "whiteYards": 392
        },
        {
          "hole": 3,
          "par": 4,
          "si": 13,
          "whiteYards": 275
        },
        {
          "hole": 4,
          "par": 3,
          "si": 17,
          "whiteYards": 111
        },
        {
          "hole": 5,
          "par": 4,
          "si": 9,
          "whiteYards": 353
        },
        {
          "hole": 6,
          "par": 5,
          "si": 1,
          "whiteYards": 550
        },
        {
          "hole": 7,
          "par": 4,
          "si": 3,
          "whiteYards": 395
        },
        {
          "hole": 8,
          "par": 3,
          "si": 15,
          "whiteYards": 128
        },
        {
          "hole": 9,
          "par": 5,
          "si": 11,
          "whiteYards": 450
        },
        {
          "hole": 10,
          "par": 4,
          "si": 8,
          "whiteYards": 368
        },
        {
          "hole": 11,
          "par": 4,
          "si": 6,
          "whiteYards": 392
        },
        {
          "hole": 12,
          "par": 4,
          "si": 14,
          "whiteYards": 275
        },
        {
          "hole": 13,
          "par": 3,
          "si": 18,
          "whiteYards": 111
        },
        {
          "hole": 14,
          "par": 4,
          "si": 10,
          "whiteYards": 353
        },
        {
          "hole": 15,
          "par": 5,
          "si": 2,
          "whiteYards": 550
        },
        {
          "hole": 16,
          "par": 4,
          "si": 4,
          "whiteYards": 395
        },
        {
          "hole": 17,
          "par": 3,
          "si": 16,
          "whiteYards": 128
        },
        {
          "hole": 18,
          "par": 5,
          "si": 12,
          "whiteYards": 450
        }
      ],
      "strokeIndexVerified": true,
      "note": "Playability nine played twice (par 72). Card prints one 9-hole SI; 18-hole SI derived by front-loop-odd / second-loop-even interleave. 9-hole White 34.5/124 drives per-loop scramble & alternate shot.",
      "source": "Cascades printed scorecard (9-hole White 34.5/124)"
    },
    "blackbear": {
      "courseId": "blackbear",
      "name": "Black Bear Golf Club",
      "location": "Franklin, NJ",
      "designer": "Kurlander/Glenz",
      "holesCount": 18,
      "tees": [
        {
          "name": "Black",
          "cr": 72.4,
          "slope": 129,
          "par": 72,
          "yards": 6673
        },
        {
          "name": "Blue",
          "cr": 70.7,
          "slope": 126,
          "par": 72,
          "yards": 6334
        },
        {
          "name": "White",
          "cr": 68.6,
          "slope": 123,
          "par": 72,
          "yards": 5883
        },
        {
          "name": "Red",
          "cr": 66.7,
          "slope": 114,
          "par": 72,
          "yards": 4785
        }
      ],
      "playedTee": "White",
      "par": 72,
      "holes": [
        {
          "hole": 1,
          "par": 4,
          "si": 9
        },
        {
          "hole": 2,
          "par": 5,
          "si": 11
        },
        {
          "hole": 3,
          "par": 4,
          "si": 1
        },
        {
          "hole": 4,
          "par": 4,
          "si": 5
        },
        {
          "hole": 5,
          "par": 3,
          "si": 17
        },
        {
          "hole": 6,
          "par": 5,
          "si": 13
        },
        {
          "hole": 7,
          "par": 3,
          "si": 7
        },
        {
          "hole": 8,
          "par": 4,
          "si": 15
        },
        {
          "hole": 9,
          "par": 4,
          "si": 3
        },
        {
          "hole": 10,
          "par": 4,
          "si": 8
        },
        {
          "hole": 11,
          "par": 3,
          "si": 18
        },
        {
          "hole": 12,
          "par": 4,
          "si": 4
        },
        {
          "hole": 13,
          "par": 5,
          "si": 2
        },
        {
          "hole": 14,
          "par": 3,
          "si": 16
        },
        {
          "hole": 15,
          "par": 4,
          "si": 14
        },
        {
          "hole": 16,
          "par": 5,
          "si": 12
        },
        {
          "hole": 17,
          "par": 4,
          "si": 6
        },
        {
          "hole": 18,
          "par": 4,
          "si": 10
        }
      ],
      "strokeIndexVerified": true,
      "source": "18Birdies official scorecard"
    }
  },
  "rounds": [
    {
      "id": "R1",
      "seq": 1,
      "date": "TUE \u00b7 JUL 21",
      "courseId": "minerals",
      "playedTee": "White",
      "teeTimeTarget": "Loop 1 10:00 AM \u00b7 Loop 2 12:36 PM",
      "teeTimes": [
        {
          "label": "Loop 1",
          "time": "10:00 AM"
        },
        {
          "label": "Loop 2",
          "time": "12:36 PM"
        }
      ],
      "players": [
        "john",
        "tj",
        "mike"
      ],
      "primaryGame": "bingoBangoBongo+nines",
      "teams": [],
      "pairings": {
        "carts": [
          [
            "john",
            "tj"
          ],
          [
            "mike"
          ]
        ],
        "rationale": "Opening threesome \u2014 Brian arrives for R2. R1 is flat / stakes-only (no Cup points), so carts are set purely for pace of play. John (14) rides with TJ (23) to spread the field across carts; Mike (23) rides solo in the two-loop rotation."
      },
      "courseHandicaps": {
        "john": 14,
        "tj": 23,
        "mike": 23
      }
    },
    {
      "id": "R2",
      "seq": 2,
      "date": "WED \u00b7 JUL 22 AM",
      "courseId": "ballyowen",
      "playedTee": "White",
      "teeTimeTarget": "7:30 AM",
      "teeTimes": [
        {
          "label": "Tee",
          "time": "7:30 AM"
        }
      ],
      "players": [
        "john",
        "brian",
        "tj",
        "mike"
      ],
      "primaryGame": "fourBallMatchPlay",
      "allowance": 0.9,
      "teams": [
        {
          "name": "Team 1",
          "players": [
            "john",
            "tj"
          ]
        },
        {
          "name": "Team 2",
          "players": [
            "brian",
            "mike"
          ]
        }
      ],
      "pairings": {
        "carts": [
          [
            "john",
            "tj"
          ],
          [
            "brian",
            "mike"
          ]
        ],
        "rationale": "First team round. John and TJ are paired here by request \u2014 they ride and partner together at Ballyowen (and again at Crystal Springs in R4). The split stays balanced despite the fixed pairing: John (13, low) + TJ (23, high) at 36 combined face Brian (18) + Mike (23) at 41 \u2014 a raw gap of just 5 that the 90% four-ball allowance and best-ball scoring tighten further. Carts follow the teams so partners can strategize between shots."
      },
      "courseHandicaps": {
        "john": 13,
        "brian": 18,
        "tj": 23,
        "mike": 23
      }
    },
    {
      "id": "R3",
      "seq": 3,
      "date": "WED \u00b7 JUL 22 PM",
      "courseId": "wildturkey",
      "playedTee": "White",
      "teeTimeTarget": "1:45 PM",
      "teeTimes": [
        {
          "label": "Tee",
          "time": "1:45 PM"
        }
      ],
      "players": [
        "john",
        "brian",
        "tj",
        "mike"
      ],
      "primaryGame": "wolf",
      "allowance": 1,
      "teams": [],
      "pairings": {
        "carts": [
          [
            "john",
            "brian"
          ],
          [
            "tj",
            "mike"
          ]
        ],
        "rationale": "Wolf is an individual game \u2014 the Wolf rotates partners hole by hole, so there are no fixed teams. Carts rotate the riding partners: John + Brian and TJ + Mike are the pairing NOT used as R2 carts, so after two rounds every player has ridden with two different people. The Wolf order (TJ, Mike, Brian, John) is unaffected by cart assignment."
      },
      "courseHandicaps": {
        "john": 19,
        "brian": 24,
        "tj": 30,
        "mike": 30
      }
    },
    {
      "id": "R4",
      "seq": 4,
      "date": "THU \u00b7 JUL 23 AM",
      "courseId": "crystalsprings",
      "playedTee": "White",
      "teeTimeTarget": "7:50 AM",
      "teeTimes": [
        {
          "label": "Tee",
          "time": "7:50 AM"
        }
      ],
      "players": [
        "john",
        "brian",
        "tj",
        "mike"
      ],
      "primaryGame": "fourBallAggregateStableford",
      "allowance": 0.85,
      "teams": [
        {
          "name": "Team 1",
          "players": [
            "john",
            "tj"
          ]
        },
        {
          "name": "Team 2",
          "players": [
            "brian",
            "mike"
          ]
        }
      ],
      "pairings": {
        "carts": [
          [
            "john",
            "tj"
          ],
          [
            "brian",
            "mike"
          ]
        ],
        "rationale": "New teams to keep partnerships balanced: John (17) + TJ (28) vs Brian (22) + Mike (28). Pairing the lowest with a highest on each side (45 vs 50 combined) is deliberately balanced for aggregate Stableford, where both partners' points count. Carts follow the teams (John + TJ, Brian + Mike) \u2014 the only cart pairing not yet used, so every pair has now shared a cart exactly once."
      },
      "courseHandicaps": {
        "john": 17,
        "brian": 22,
        "tj": 28,
        "mike": 28
      }
    },
    {
      "id": "R5",
      "seq": 5,
      "date": "THU \u00b7 JUL 23 PM",
      "courseId": "cascades",
      "playedTee": "White",
      "teeTimeTarget": "Loop 1 2:02 PM \u00b7 Loop 2 4:08 PM",
      "teeTimes": [
        {
          "label": "Loop 1",
          "time": "2:02 PM"
        },
        {
          "label": "Loop 2",
          "time": "4:08 PM"
        }
      ],
      "players": [
        "john",
        "brian",
        "tj",
        "mike"
      ],
      "primaryGame": "scramble+alternateShot",
      "teams": [
        {
          "name": "Team 1",
          "players": [
            "john",
            "mike"
          ]
        },
        {
          "name": "Team 2",
          "players": [
            "brian",
            "tj"
          ]
        }
      ],
      "pairings": {
        "carts": [
          [
            "john",
            "mike"
          ],
          [
            "brian",
            "tj"
          ]
        ],
        "rationale": "R5 is an individual round \u2014 full-18 Bingo-Bango-Bongo plus round-robin 1v1 match play, so every player faces every other player head-to-head (all six 1v1 matches). Combined with the R2 and R4 team-round match-ups, this evens out the opponent matrix. Carts (John + Mike, Brian + TJ) keep the rotation flat \u2014 by the end of Thursday every player has still ridden with all three of the others at least once."
      },
      "courseHandicaps": {
        "john": 17,
        "brian": 22,
        "tj": 28,
        "mike": 28
      }
    },
    {
      "id": "R6",
      "seq": 6,
      "date": "FRI \u00b7 JUL 24",
      "courseId": "blackbear",
      "playedTee": "White",
      "teeTimeTarget": "8:36 AM",
      "teeTimes": [
        {
          "label": "Tee",
          "time": "8:36 AM"
        }
      ],
      "players": [
        "john",
        "brian",
        "tj",
        "mike"
      ],
      "primaryGame": "championshipSingles+stableford",
      "allowance": 1,
      "teams": [],
      "pairings": {
        "carts": [
          [
            "john",
            "tj"
          ],
          [
            "brian",
            "mike"
          ]
        ],
        "rationale": "Championship singles are seeded off the standings after R5 (1v2 for the Cup, 3v4 for the Bronze), so match opponents are only known on Friday morning. Carts (John + TJ, Brian + Mike) put John and TJ together one last time to close out the trip \u2014 their fourth ride together (R1, R2, R4, R6) at the request to keep them paired as much as possible. Every player has still shared a cart with each of the other three at least once by the end of R5, so cart coverage is complete regardless of the seeded matches. If the seeded matches differ from these carts, the higher seed's group rides together; the fairness matrix treats R6 opponents as seeded/TBD."
      },
      "courseHandicaps": {
        "john": 16,
        "brian": 22,
        "tj": 27,
        "mike": 27
      }
    }
  ],
  "strokeAllocations": {
    "R1": {
      "john": {
        "courseHandicap": 14,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 0,
            "holes": [],
            "basis": "off low, over all 18"
          },
          "nines": {
            "allowance": 1,
            "nineHoleCH": 7,
            "strokes": 0,
            "holes": [],
            "basis": "9-hole handicaps, loop 2 only; BBB (loop 1) is gross"
          }
        }
      },
      "tj": {
        "courseHandicap": 23,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 9,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "nines": {
            "allowance": 1,
            "nineHoleCH": 11,
            "strokes": 4,
            "holes": [
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "9-hole handicaps, loop 2 only; BBB (loop 1) is gross"
          }
        }
      },
      "mike": {
        "courseHandicap": 23,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 9,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "nines": {
            "allowance": 1,
            "nineHoleCH": 11,
            "strokes": 4,
            "holes": [
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "9-hole handicaps, loop 2 only; BBB (loop 1) is gross"
          }
        }
      }
    },
    "R2": {
      "john": {
        "courseHandicap": 13,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 0,
            "holes": [],
            "basis": "off low, over all 18"
          },
          "fourBallMatch": {
            "allowance": 0.9,
            "playingHandicap": 12,
            "strokes": 0,
            "holes": [],
            "basis": "90% off low"
          }
        }
      },
      "brian": {
        "courseHandicap": 18,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 5,
            "holes": [
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "fourBallMatch": {
            "allowance": 0.9,
            "playingHandicap": 16,
            "strokes": 4,
            "holes": [
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "90% off low"
          }
        }
      },
      "tj": {
        "courseHandicap": 23,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 10,
            "holes": [
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "fourBallMatch": {
            "allowance": 0.9,
            "playingHandicap": 21,
            "strokes": 9,
            "holes": [
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "90% off low"
          }
        }
      },
      "mike": {
        "courseHandicap": 23,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 10,
            "holes": [
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "fourBallMatch": {
            "allowance": 0.9,
            "playingHandicap": 21,
            "strokes": 9,
            "holes": [
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "90% off low"
          }
        }
      }
    },
    "R3": {
      "john": {
        "courseHandicap": 19,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 0,
            "holes": [],
            "basis": "off low, over all 18"
          },
          "wolf": {
            "allowance": 1,
            "strokes": 0,
            "holes": [],
            "basis": "100% off low; best net ball"
          }
        }
      },
      "brian": {
        "courseHandicap": 24,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 5,
            "holes": [
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "wolf": {
            "allowance": 1,
            "strokes": 5,
            "holes": [
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              }
            ],
            "basis": "100% off low; best net ball"
          }
        }
      },
      "tj": {
        "courseHandicap": 30,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "wolf": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "100% off low; best net ball"
          }
        }
      },
      "mike": {
        "courseHandicap": 30,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "wolf": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "100% off low; best net ball"
          }
        }
      }
    },
    "R4": {
      "john": {
        "courseHandicap": 17,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 0,
            "holes": [],
            "basis": "off low, over all 18"
          },
          "teamStableford": {
            "allowance": 0.85,
            "playingHandicap": 14,
            "strokes": 14,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "85% full dots"
          }
        }
      },
      "brian": {
        "courseHandicap": 22,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 5,
            "holes": [
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "teamStableford": {
            "allowance": 0.85,
            "playingHandicap": 19,
            "strokes": 19,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 2
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "85% full dots"
          }
        }
      },
      "tj": {
        "courseHandicap": 28,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "teamStableford": {
            "allowance": 0.85,
            "playingHandicap": 24,
            "strokes": 24,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 2
              },
              {
                "hole": 3,
                "strokes": 2
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 2
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 2
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 2
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 2
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "85% full dots"
          }
        }
      },
      "mike": {
        "courseHandicap": 28,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "teamStableford": {
            "allowance": 0.85,
            "playingHandicap": 24,
            "strokes": 24,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 2
              },
              {
                "hole": 3,
                "strokes": 2
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 2
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 2
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 2
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 2
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "85% full dots"
          }
        }
      }
    },
    "R5": {
      "john": {
        "courseHandicap": 17,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 0,
            "holes": [],
            "basis": "off low, over all 18"
          }
        }
      },
      "brian": {
        "courseHandicap": 22,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 5,
            "holes": [
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          }
        }
      },
      "tj": {
        "courseHandicap": 28,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          }
        }
      },
      "mike": {
        "courseHandicap": 28,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          }
        }
      },
      "_teamHandicaps": {
        "nineHoleCH": {
          "john": 8,
          "brian": 11,
          "tj": 14,
          "mike": 14
        },
        "scramble": {
          "allowance": "35% low + 15% high",
          "team1": 5,
          "team2": 6,
          "basis": "net stroke play per loop"
        },
        "alternateShot": {
          "allowance": "50% combined",
          "team1": 11,
          "team2": 13,
          "strokesToTeam2": 2,
          "team2StrokeHolesLoop1": [
            6,
            7
          ],
          "team2StrokeHolesLoop2": [
            15,
            16
          ],
          "basis": "match play over 9; higher team gets the difference on the loop's lowest 9-hole-SI holes"
        }
      }
    },
    "R6": {
      "john": {
        "courseHandicap": 16,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 0,
            "holes": [],
            "basis": "off low, over all 18"
          },
          "stableford": {
            "allowance": 1,
            "strokes": 16,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "100% full dots"
          }
        }
      },
      "brian": {
        "courseHandicap": 22,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 6,
            "holes": [
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "stableford": {
            "allowance": 1,
            "strokes": 22,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 2
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 2
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 2
              },
              {
                "hole": 13,
                "strokes": 2
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "100% full dots"
          }
        }
      },
      "tj": {
        "courseHandicap": 27,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "stableford": {
            "allowance": 1,
            "strokes": 27,
            "holes": [
              {
                "hole": 1,
                "strokes": 2
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 2
              },
              {
                "hole": 4,
                "strokes": 2
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 2
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 2
              },
              {
                "hole": 10,
                "strokes": 2
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 2
              },
              {
                "hole": 13,
                "strokes": 2
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 2
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "100% full dots"
          }
        }
      },
      "mike": {
        "courseHandicap": 27,
        "games": {
          "skinsNet": {
            "allowance": 1,
            "strokes": 11,
            "holes": [
              {
                "hole": 1,
                "strokes": 1
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 1
              },
              {
                "hole": 4,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 1
              },
              {
                "hole": 10,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 1
              },
              {
                "hole": 13,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 1
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "off low, over all 18"
          },
          "stableford": {
            "allowance": 1,
            "strokes": 27,
            "holes": [
              {
                "hole": 1,
                "strokes": 2
              },
              {
                "hole": 2,
                "strokes": 1
              },
              {
                "hole": 3,
                "strokes": 2
              },
              {
                "hole": 4,
                "strokes": 2
              },
              {
                "hole": 5,
                "strokes": 1
              },
              {
                "hole": 6,
                "strokes": 1
              },
              {
                "hole": 7,
                "strokes": 2
              },
              {
                "hole": 8,
                "strokes": 1
              },
              {
                "hole": 9,
                "strokes": 2
              },
              {
                "hole": 10,
                "strokes": 2
              },
              {
                "hole": 11,
                "strokes": 1
              },
              {
                "hole": 12,
                "strokes": 2
              },
              {
                "hole": 13,
                "strokes": 2
              },
              {
                "hole": 14,
                "strokes": 1
              },
              {
                "hole": 15,
                "strokes": 1
              },
              {
                "hole": 16,
                "strokes": 1
              },
              {
                "hole": 17,
                "strokes": 2
              },
              {
                "hole": 18,
                "strokes": 1
              }
            ],
            "basis": "100% full dots"
          }
        }
      },
      "_singles": {
        "seeding": "EGT Cup standings after R5 \u2014 1v2 Championship, 3v4 Bronze",
        "allowance": 1,
        "courseHandicaps": {
          "john": 16,
          "brian": 22,
          "tj": 27,
          "mike": 27
        },
        "note": "Higher-CH player gets (CH difference) strokes on lowest-SI holes. Diffs \u2014 Brian-John 6 \u00b7 TJ/Mike-John 11 \u00b7 TJ/Mike-Brian 5 \u00b7 TJ-Mike 0."
      }
    }
  },
  "formatConfigs": {
    "R1": {
      "loop1": {
        "game": "bingoBangoBongo",
        "scoring": "gross",
        "pointsPerHole": 3
      },
      "loop2": {
        "game": "nines",
        "basis": "9-hole handicaps",
        "pointsPerHole": 9,
        "split": [
          5,
          3,
          1
        ],
        "tieRule": "split evenly"
      },
      "skins": {
        "net": true,
        "gross": true,
        "over": "all 18"
      }
    },
    "R2": {
      "game": "fourBallMatchPlay",
      "allowance": 0.9,
      "nassau": {
        "front": 1,
        "back": 1,
        "overall": 2
      }
    },
    "R3": {
      "game": "wolf",
      "order": [
        "tj",
        "mike",
        "brian",
        "john"
      ],
      "units": {
        "pack2v2": 1,
        "loneWolf": 2,
        "blindWolf": 3
      },
      "perOpponent": true,
      "ties": "push",
      "holes17_18": "trailing in units is Wolf on 17, next-to-last on 18"
    },
    "R4": {
      "game": "fourBallAggregateStableford",
      "allowance": 0.85,
      "stablefordPoints": {
        "netEagle": 4,
        "netBirdie": 3,
        "netPar": 2,
        "netBogey": 1,
        "netDoubleOrWorse": 0
      },
      "segments": [
        [
          1,
          6
        ],
        [
          7,
          12
        ],
        [
          13,
          18
        ]
      ],
      "pickUp": "at net double bogey"
    },
    "R5": {
      "loop1": {
        "game": "scramble2",
        "allowance": {
          "low": 0.35,
          "high": 0.15
        },
        "scoring": "net stroke play"
      },
      "loop2": {
        "game": "alternateShot",
        "allowance": 0.5,
        "scoring": "match play over 9"
      }
    },
    "R6": {
      "singles": {
        "seeding": "EGT Cup standings after R5 \u2014 1v2 Championship, 3v4 Bronze",
        "allowance": 1
      },
      "stableford": {
        "allowance": 1,
        "fullDots": true
      }
    }
  },
  "sideGames": {
    "skins": {
      "pots": [
        "gross",
        "net"
      ],
      "carryovers": true,
      "antePerRoundPerPot": 5
    },
    "closestToPin": {
      "rounds": [
        "R2",
        "R3",
        "R4",
        "R5",
        "R6"
      ],
      "rule": "all par 3s; on green",
      "value": 5
    },
    "longDrive": {
      "rule": "one hole/round \u2014 longest par 5 on card; must finish in fairway",
      "value": 5
    },
    "passTheMoney": {
      "token": "The Rock \u2014 $20 bill",
      "startHolder": "john",
      "startsAt": "R2 hole 1",
      "steal": "make a net birdie",
      "potRule": "every 3-putt adds $1",
      "endRule": "holder after R6 h18 keeps bill + pot",
      "ledgerFields": [
        "round",
        "hole",
        "from",
        "to",
        "reason",
        "potTotal"
      ]
    },
    "trackedStats": [
      "putts",
      "fairwaysHit",
      "greensInReg",
      "grossBirdies",
      "netBirdies",
      "pars",
      "sandSaves"
    ]
  },
  "pointsConfig": {
    "R1": {
      "bbbChampion": 3,
      "ninesChampion": 3
    },
    "R2": {
      "nassauFront": 1,
      "nassauBack": 1,
      "nassauOverall": 2
    },
    "R3": {
      "wolfFinish": [
        4,
        3,
        2,
        1
      ]
    },
    "R4": {
      "segmentWin": 1,
      "overall18": 2
    },
    "R5": {
      "scrambleLoop": 2,
      "alternateShotLoop": 2
    },
    "R6": {
      "singlesWin": 3,
      "singlesHalve": 1.5,
      "stablefordFinish": [
        4,
        3,
        2,
        1
      ]
    },
    "seasonAwards": {
      "skinsKing": 2,
      "birdieKingGross": 4,
      "parKing": 2,
      "bogeyGod": 1,
      "flatStickFewestPutts": 1,
      "ironManFIRplusGIR": 1
    },
    "tiebreakers": [
      "R6 stableford points",
      "head-to-head matches",
      "total skins",
      "chip-off at Black Bear"
    ],
    "maxPossible": {
      "john": 35,
      "tj": 35,
      "mike": 35,
      "brian": 35
    }
  },
  "moneyDefaults": {
    "nassauPerPoint": 5,
    "wolfPerUnit": 2,
    "bbbNinesPerPointDiff": 1,
    "skinsAnte": 5,
    "ctpLd": 5,
    "ptmBill": 20
  }
};
if (typeof window !== "undefined") { window.EGT_SEED = EGT_SEED; }
