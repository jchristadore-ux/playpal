...
const SummaryScreen = ({ round, scores, wolfData, putts, nassauPresses, manualChips, onNewRound }) => {
  ...
  // ── Email ─────────────────────────────────────────────────────────────────
  // Opens the device's native email client with a pre-filled scorecard.
  // Full server-side email requires a backend (see README for setup steps).
  const sendEmail = () => {
    const emails = players.map(p => p.email).filter(Boolean);
    if (emails.length === 0) {
      showToast('No email addresses in player profiles', 'error');
      return;
    }
    const subject = `PlayPal Scorecard: ${course.name}`;
    const lines = [
      'PlayPal Golf Scorecard',
      `${course.name} · ${new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'})}`,
      ...
      ...players.map(p => {
        ...
        return `  ${p.name}: ${gross || '–'} (${vs===0?'E':vs>0?'+'+vs:vs})`;
      }),
      '',
      'SETTLEMENT',
      ...(debts.length === 0
        ? ['  All square — no money changes hands']
        : debts.map(d => `  ${d.from.name} → ${d.to.name}: $${d.amount.toFixed(0)}`)),
      '',
      'Generated by PlayPal',
    ];
    window.open(
      `mailto:${emails.join(',')}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(lines.join('\n'))}`
    );
    setEmailSent(true);
    showToast('Email client opened — compose and send');
  };
  ...
  const generatePDF = () => {
    ...
    doc.text('PlayPal Golf Scorecard', margin, y); y += 8;
    doc.setFontSize(12); doc.setFont('helvetica', 'normal');
    doc.text(`${course.name} · ${new Date().toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'})}`, margin, y); y += 6;
    doc.text(`Formats: ${round.formats.map(f=>FORMAT_INFO[f.type].label).join(' · ')}`, margin, y); y += 10;

    // Scorecard table
    const scHeader = ['Player', ...course.holes.map(h=>`H${h.num}`), 'Total', '+/−'];
    const scBody = [
      ['PAR', ...course.holes.map(h=>String(h.par)), String(course.holes.reduce((a,h)=>a+h.par,0)), ''],
      ...players.map(p => {
        const gross = totalScore(scores, p.id);
        const vs = totalVsPar(scores, p.id, course.holes);
        return [
          p.name,
          ...course.holes.map((_,i) => String(scores[p.id]?.[i] || '–')),
          gross ? String(gross) : '–',
    ...
    if (doc.autoTable) {
      doc.autoTable({
        ...
        alternateRowStyles: { fillColor: [15,32,64] },
    ...
    // Wolf standings
    if (Object.keys(wolfPts).length > 0) {
      doc.setFontSize(14); doc.setFont('helvetica', 'bold');
      doc.text('Wolf Standings', margin, y); y += 7;
      doc.setFontSize(10); doc.setFont('helvetica', 'normal');
      players.forEach(p => {
        doc.text(`${p.name}: ${wolfPts[p.id]||0} pts`, margin, y); y += 5;
      });
      y += 3;
    }
    ...
    if (ptmState.log?.length > 0) {
      doc.setFontSize(14); doc.setFont('helvetica', 'bold');
      doc.text('Pass The Money', margin, y); y += 7;
      doc.setFontSize(10); doc.setFont('helvetica', 'normal');
      ptmState.log.slice(0, 18).forEach(entry => {
        const holder = players.find(p => p.id === entry.holderId);
        if (holder) { doc.text(`H${(entry.hole||0)+1}: ${holder.name}`, margin, y); y += 5; }
      });
      y += 3;
    }
    ...
    // Net settlement
    doc.setFontSize(14); doc.setFont('helvetica', 'bold');
    doc.text('Net Settlement', margin, y); y += 7;
    doc.setFontSize(10); doc.setFont('helvetica', 'normal');
    if (debts.length === 0) {
      doc.text('All square — no money changes hands', margin, y);
    } else {
      debts.forEach(d => {
        doc.text(`${d.from.name}  →  ${d.to.name}:  $${d.amount.toFixed(0)}`, margin, y); y += 5;
      });
    }

    doc.save(`playpal-${course.name.replace(/[^a-z0-9]/gi,'-').toLowerCase()}.pdf`);
    showToast('PDF downloaded');
  };

  const tabs = [['scorecard','📊 SCORES'],['payouts','💰 PAYOUTS'],['actions','📤 SEND'],['pdf','📄 PDF']];
    ...
    <div style={sumS.root}>
      ...
      <div style={sumS.content}>
        ...
        {tab==='actions' && (
          <div style={{display:'flex', flexDirection:'column', gap:10}}>
            ...
            <div style={{background:'#0F2040', border:'1px solid #1E3A6E', borderRadius:12, padding:'16px'}}>
              ...
              <div style={{fontFamily:'DM Sans', fontSize:12, color:'#7A98BC', marginBottom:12}}>Opens your email app pre-filled with the scorecard for all {players.length} players</div>
        ...
        {tab==='pdf' && (
          <div style={{display:'flex', flexDirection:'column', gap:12}}>
            <div style={{background:'#0F2040', border:'1px solid #1E3A6E', borderRadius:12, padding:'16px'}}>
              <div style={{fontFamily:'Barlow Condensed', fontWeight:700, fontSize:16, color:'#fff', marginBottom:4}}>📄 SCORECARD PDF</div>
              <div style={{fontFamily:'DM Sans', fontSize:12, color:'#7A98BC', marginBottom:16, lineHeight:1.6}}>
                Downloads a fully filled-out scorecard PDF including strokes per hole,
                Wolf standings, PTM movement, and net settlement.
              </div>
              <div style={{display:'flex', flexDirection:'column', gap:6, marginBottom:16}}>
                {[
                  '✓ Strokes per hole for each player',
                  '✓ Total score and vs-par',
                  '✓ Wolf points per hole and totals',
                  '✓ Pass The Money movement log',
                  '✓ Money owed between players',
                ].map(item=>(
                  <div key={item} style={{fontSize:12, color:'#9BB4D4', fontFamily:'DM Sans'}}>{item}</div>
                ))}
              </div>
              <Btn onClick={generatePDF} variant="gold" style={{width:'100%', fontSize:15}}>
                📥 DOWNLOAD PDF
              </Btn>
            </div>
          </div>
