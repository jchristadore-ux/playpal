...
const SummaryScreen = ({ round, scores, wolfData, putts, nassauPresses, manualChips, popFlags, bbbData, teeBallData, firData, girData, onNewRound, readOnly }) => {
  ...
  const payouts = React.useMemo(() => {
    try {
      return calcAllPayouts(_scores, _wolfData, players, course, formats, _presses, ptmState.holderId, _popFlags, null, _bbbData, _teeBallData);
    } catch(e) {
      console.warn('[Summary] calcAllPayouts failed:', e);
  ...
  const buildScorecardEmail = () => {
    ...
    players.forEach(p => {
      ...
      course.holes.forEach((h,i) => { const s=_scores[p.id]?.[i]; body += pad(s||'·', 4); });
      const tot = totalScore(_scores, p.id);
    ...
    if (formats.length > 0) {
      ...
      formats.forEach((f, fi) => {
        ...
        if (f.type === 'skins') {
          const { skins } = calcSkins(_scores, players, course, f.stakes, _popFlags);
          players.forEach(p => {
            const n = skins[p.id] || 0;
            body += lpad(p.name, nameW+2) + lpad(`${n} skin${n!==1?'s':''}`, 12) + pad(fmtPay(fPay[p.id]||0), 8) + '\n';
          });
        } else if (f.type === 'wolf') {
          players.forEach(p => {
            body += lpad(p.name, nameW+2) + lpad(`${wolfPts[p.id]||0} pts`, 12) + pad(fmtPay(fPay[p.id]||0), 8) + '\n';
          });
          ...
          matches.forEach(match => {
            if (!match) return;
            const mPlayers = (match.playersInMatch || []).map(id => players.find(p => p.id === id)).filter(Boolean);
            const mNames   = mPlayers.length >= 2 ? mPlayers.map(p=>p.name.split(' ')[0]).join(' vs ') : players.slice(0,2).map(p=>p.name.split(' ')[0]).join(' vs ');
            const ms = match.stakes ?? f.stakes;
            body += `Match: ${mNames}\n`;
            const front = nassauSegmentStatus(_scores, players, course, range9f, 8,  _popFlags, match);
            const back  = nassauSegmentStatus(_scores, players, course, range9b, 17, _popFlags, match);
            const total = nassauSegmentStatus(_scores, players, course, range18, 17, _popFlags, match);
          ...
          players.forEach(p => { body += lpad(p.name, nameW+2) + pad(fmtPay(fPay[p.id]||0), 8) + '\n'; });
        } else if (f.type === 'stableford') {
          players.forEach(p => { body += lpad(p.name, nameW+2) + lpad(`${stablefordPts[p.id]||0} pts`, 12) + pad(fmtPay(fPay[p.id]||0), 8) + '\n'; });
          ...
          if (ptmState.log.length > 0) {
            ...
            ptmState.log.slice(0, 6).forEach(ev => {
              ...
              if (from && to) body += `  Hole ${ev.holeIdx+1}: ${from.name.split(' ')[0]} → ${to.name.split(' ')[0]} (${ev.reason})\n`;
            });
          }
          const winner = players.find(p => p.id === ptmState.holderId);
          if (winner) body += `Final holder: ${winner.name} wins $${(fPay[ptmState.holderId]||0).toFixed(0)}\n`;
        } else if (f.type === 'bingobangobongo') {
          players.forEach(p => {
            const st = bbbStandings[p.id] || {bingo:0,bango:0,bongo:0,total:0};
            body += lpad(p.name, nameW+2) + lpad(`${st.total}pts (${st.bingo}/${st.bango}/${st.bongo})`, 18) + pad(fmtPay(fPay[p.id]||0), 8) + '\n';
          });
        } else if (f.type === 'teeball') {
          players.forEach(p => {
            body += lpad(p.name, nameW+2) + lpad(`${teeBallStandings[p.id]||0} tee ball pts`, 16) + pad(fmtPay(fPay[p.id]||0), 8) + '\n';
          });
          ...
          players.forEach(p => {
            ...
            body += lpad(p.name, nameW+2) + lpad(`Team ${team}`, 10) + pad(fmtPay(fPay[p.id]||0), 8) + '\n';
          });
        }
      });
      ...
      if (debts.length === 0) {
        ...
        debts.forEach(d => { body += `${d.from.name} owes $${d.amount.toFixed(0)} to ${d.to.name}\n`; });
      }
    }
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      {course.holes.slice(9,18).map((h,i)=>{
                        ...
                        const c=d===null?'#E7E3D9':d<=-2?'#C8A15A':d===-1?'#15803D':d===0?'#6B7280':d===1?'#DC2626':'#991B1B';
                        const hasPop = !!(_popFlags?.[p.id]?.[ri]);
                        return (
                ...
                <div style={{display:'flex', flexDirection:'column', gap:6}}>
                  {players.map(p=>(
                    <div key={p.id} style={{display:'flex', alignItems:'center', gap:8, padding:'6px 0', borderBottom:'1px solid #F0EDE4'}}>
                      ...
                      <span style={{fontFamily:'Plus Jakarta Sans, Inter, system-ui, sans-serif', fontSize:13, color:'#15803D', fontWeight:700}}>✓ {totalScore(_scores,p.id)||'—'}</span>
                    </div>
                  ))}
                </div>
              )}
