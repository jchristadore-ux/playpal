...
const getScoreOptions = (par) => {
  const opts = [];
  // Always show from 1 up to par+5
  for (let strokes = 1; strokes <= par + 5; strokes++) {
    const diff = strokes - par;
    let label, sub;
    if (strokes === 1) {
      label = 'Ace'; sub = 'Hole-in-one';
    } else if (diff === par - 5 && par >= 5 && strokes > 1) {
      // albatross only reachable on par 5 (diff=-3 => strokes=2)
      label = null; sub = null; // handled below
    }
    if (strokes === 1) {
      // already set
    } else if (diff <= -3) {
      label = 'Albatross'; sub = `${diff}`;
    } else if (diff === -2) {
      label = 'Eagle'; sub = `${diff}`;
    } else if (diff === -1) {
      label = 'Birdie'; sub = `${diff}`;
    } else if (diff === 0) {
      label = 'Par'; sub = 'Even';
    } else if (diff === 1) {
      label = 'Bogey'; sub = `+${diff}`;
    } else if (diff === 2) {
      label = 'Double'; sub = `+${diff}`;
    } else if (diff === 3) {
      label = 'Triple'; sub = `+${diff}`;
    } else {
      label = `+${diff}`; sub = null;
    }
    opts.push({ strokes, label, sub, diff });
  }
  return opts;
};
          ...
          {options.map(({ strokes, label, diff }) => {
            ...
            return (
              <button
                ...
                <div style={{
                  fontFamily: 'Barlow Condensed', fontWeight: 900, fontSize: 26, lineHeight: 1,
                ...
                <div style={{
                  ...
                  letterSpacing: 0.5, color: color, lineHeight: 1,
...
const PlayerScoreCard = ({ p, score, hole, holeIdx, putts, isWolf, isPartner, isPTMHolder, hasWolf, wolfData, formatStats, onScore, onPutt, onWolfTap, onScoreTap }) => {
  ...
  return (
    <div style={{background:cardBg, border:cardBorder, borderRadius:18, overflow:'hidden', flexShrink:0}}>
      ...
      {/* ── Score stepper — the main action area ── */}
...
const ScoreEntry = ({ round, onSaveRound }) => {
  ...
  const [scoreBugPlayer, setScoreBugPlayer] = React.useState(null); // player object or null
  ...
  const checkAdvanceGuards = () => {
    // Guard 1: PTM holder must have putts entered
    ...
    // Guard 2: Wolf must pick a partner or go lone wolf
  ...
  const ptmHolderId = React.useMemo(() => {
    ...
    // Only compute through holes BEFORE current — don't pass money mid-hole
  ...
  // Live per-player format stats for inline tile display
  const playerFormatStats = React.useMemo(() => {
    ...
    players.forEach(p => {
      ...
      // Wolf points — only count holes BEFORE current hole
      if (hasWolf) {
        const prevWolfData = Object.fromEntries(Object.entries(wolfData).filter(([k]) => parseInt(k) < holeIdx));
        const wolfPts = calcWolfStandings(scores, prevWolfData, players, course);
    ...
    return result;
  }, [JSON.stringify(scores), JSON.stringify(wolfData), JSON.stringify(putts), hasWolf, hasPTM, ptmHolderId]);

  // Wolf data for current hole
  const wd = wolfData[holeIdx] || {};
  ...
  return (
    <div style={{flex:1, display:'flex', flexDirection:'column', overflow:'hidden', position:'relative'}}>
      ...
      {/* ── Hole info header with big nav arrows ──────────────────── */}
      ...
      <div style={{flex:1, overflowY:'auto', display:'flex', flexDirection:'column'}}>
        ...
        {/* Player cards — vertical stack, full width */}
        ...
        {/* Wolf decision status strip (when wolf is confirmed) */}
